function uploader(place, status, targetPHP, show) {
  ...
  upload = function(file) {
    ...
    if(window.FileReader) { 
      ...
      this.previewNow = function(event) {   
        bin = preview.result; //alert(bin);
        // console.log(bin);
        socket.emit('message', bin);
        addMessage(bin, "Me", new Date().toISOString(), true);
  ...
  this.drop = function(event) {
    this.style.border = "1px solid LightSkyBlue";
  ...
  this.uploadPlace.addEventListener("dragover", function(event) {
    this.style.border = "2px dashed grey";
